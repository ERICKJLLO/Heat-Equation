...
int main() {
    ...
    FILE *output = fopen("heat2d_results_secuencial.csv", "w");
    if (!output) {
        printf("Error: No se pudo abrir heat2d_results_secuencial.csv para escritura.\n");
        for (int i = 0; i < NX; i++) {
            free(u[i]);
            free(u_new[i]);
        }
